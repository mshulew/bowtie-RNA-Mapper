...
handles barcode SEQuoia Express or NEB
"""
...
# move barcode from R2 to read name
    good_counter = 0
    bad_counter = 0

    for a in range(0,len(input_file_data),4):
        if library_type == 'sequoiaexpress':
            barcode = input_file_data[a+1].split('\t')[1][0:8]
            missing = 8 - len(barcode)
        elif library_type == 'neb':
            barcode = input_file_data[a+1].split('\t')[1][0:11]
            missing = 11 - len(barcode)
